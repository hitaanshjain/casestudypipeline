...
def build_schema():
    return {
        ...
        "description": "Enforced by tools/check_flashcard_json.py, which is the "
                       ...
                       "but NOT sufficient. Not expressed at all: G03 "
                       "(word-count budgets on seven fields), G06 (exactly "
                       "one bold row, must be last), G07 (aligned rows must "
                       "be contiguous and run to the end), G09 (variable_key "
                       "must cover every symbol in front.central), G10 (a "
                       "back row may not use a symbol the front or an "
                       "earlier row never introduced), G11 (no Unicode math "
                       "glyphs in prose fields), G12 (balanced LaTeX "
                       "braces), G14 (banned footer prefixes). Partially "
                       "expressed: G08 (variable_key's shape and max entry "
                       "count are checked here; its presence being tied to "
                       "front.central's latex/text choice, symbol "
                       "uniqueness, and the per-entry meaning word cap are "
                       "not); G13 (the null-lo_ordinal branch is checked via "
                       "source's if/then; the non-null branch, lo_ordinal "
                       "range and lo_text matching the real corpus "
                       "objective, needs a live references/*/book_map.json "
                       "lookup a static schema cannot perform). A card can "
