...
Emits the subject/textbook/chapter/learning_objective hierarchy only; no
concept or card rows. Run from repo root:  python tools/generate_flashcards_seed.py
"""
import json
...
def build_sql():
    """The full seed SQL text, in memory. main() writes it to OUT; no drift test
    exists against the committed file (test_import_concept_flashcards.py, which
    used to do that, was deleted with the rest of the concept-card import path)."""
    ...
    return "\n".join(lines), len(chapter_rows), lo_id


def main():
    sql, n_chapters, n_los = build_sql()
    OUT.write_text(sql, encoding="utf-8", newline="\n")
    print(f"chapters={n_chapters} los={n_los}")
