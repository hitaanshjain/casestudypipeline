...
- Notation that every student already reads needs no entry and should not get one. Spending an entry on any of these wastes one of your five: + - = \cdot \times \div \pm \mp \frac \sqrt \left \right \big \Big \bigg \Bigg \quad \qquad \, \; \text \displaystyle \boldsymbol \bm \approx \neq \leq \geq \to \infty \ldots \cdots and the standard named functions \sin \cos \tan \sec \csc \cot \log \ln \exp. Letters sitting inside \text{...} are words, not variables, and need no entry either.
...
- No LaTeX environment goes in central.latex or in any row: no \begin{aligned}, no \begin{array}, no \begin{cases}, none at all. Rows carry structure in this format, not LaTeX: each row is its own object and each row's aligned boolean is what lines the derivation up. An environment name is also read letter by letter, so \begin{aligned} alone demands key entries for a, d, e, g, i, l, and n, seven junk letters against a cap of five, and the card cannot pass. One relation per row, no environments.
