...
```json
{
  ...
  "problem": {
    ...
    "latex": "string LaTeX, the problem's mathematical givens stated formally (defining equations, domain, key constants); never task sentences",
...
Too long: "Introduce x and y for the fence segments and write the fixed-fencing constraint alongside the area to be maximized." Say "Name the variables and write the fencing constraint." instead.
...
- an equation NEVER carries its own verbal conclusion or context. Never follow \Rightarrow (or \implies, \therefore) with \text{...}: write the math result as the equation and state the conclusion in the CAPTION ("So \(f'(0)\) does not exist."). Never open an equation with a \text context clause and a colon: "\text{at } h = 0.00001: \dfrac{h^{1/5}}{h} = ..." breaks onto lines mid-thought; put the context in the caption or the entry's plain-text label ("CHECK AT h = 0.00001") and keep the latex to the bare computation. No colons in equation latex at all. Never justify inside math: \boxed{f'(0) \text{ does not exist because } \lim ...} is wrong; box the bare conclusion \boxed{f'(0) \text{ does not exist}} and let the caption carry the reason. (\text{} for units and short labels like "does not exist" ALONE stays fine.)
...
- problem.latex states the problem's GIVENS formally for the side panel: the defining equation(s), the domain or interval, and any key constant, comma-separated, for example g(t) = 6t - t^{2}, \quad 0 \le t \le 6, \quad \text{capacity } 30 \text{ liters}. It contains NO task language: words like show, find, evaluate, determine, prove, compute, using never appear there, not even inside \text{} (units and labels in \text{} are fine); every task lives in problem.prompt. It adds no given that problem.prompt omitted. Task sentences in math mode render as one long line-broken equation of centered prose fragments, which is unreadable beside the deck.
- Every mathematical expression inside ANY prose field is wrapped in \( \) inline delimiters: problem.prompt, the deck title and subtitle, every step title, every step caption, callout text, visual captions, plot curve and shade labels, and reference.equations titles and text. Write "Analyze the Sign of \(h^{2/3}\)", never "Analyze the Sign of h^{2/3}". The renderer typesets only delimited math, so an undelimited expression displays to the student as raw source characters.
- EXCEPTION, plain-text-only fields: plot mark labels, number-line point and interval labels, table column headers, equation labels (equations[].label), and side-card labels (cards[].label) are rendered as plain text (figure paint or uppercase chips) and CANNOT render LaTeX at all. No backslashes, no braces, no \( \) there, ever. Write them plainly ("h = 0", "h^(2/3) > 0", "SUBSTITUTE") or as words ("positive", "undefined"). A label like "h^{2/3} > 0" ships to the student as literal brace soup.
