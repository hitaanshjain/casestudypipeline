...
- EXCEPTION, labels drawn INSIDE a figure: plot mark labels, number-line point and interval labels, and table column headers are painted as plain text into the drawing and CANNOT render LaTeX at all. No backslashes, no braces, no \( \) there, ever. Write the label plainly ("h = 0", "h^(2/3) > 0") or use a word ("positive", "undefined"). A label like "h^{2/3} > 0" ships to the student as literal brace soup.
