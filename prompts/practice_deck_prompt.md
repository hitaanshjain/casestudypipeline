...
- Every mathematical expression inside problem.prompt, a step caption, or a callout's text is wrapped in \( \) inline delimiters: write "\(d(t) = (t - 4)^{1/3}\) meters", never "d(t) = (t - 4)^{1/3} meters". The renderer typesets only delimited math, so an undelimited expression displays to the student as raw source characters.
