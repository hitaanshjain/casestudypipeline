...
- COMPLETENESS: after the top-to-bottom walk, check the reverse direction. Every part your independent solution answers must have a final answer stated in verified_answer.txt; an absent part-final is a calibration mismatch, reported with the same error line format and the generator's value given as not stated (<y> = not stated). A generator that silently dropped a part does not pass just because everything it did state matches.
...
A6 verified_answer.txt density. Phase 2 uses verified_answer.txt as its method exemplar, so density matters beyond correctness: does it show the operator-bearing steps for every part, or only final results? Results-only prose, or a part whose steps a reader could not recompute the answer from, is a finding; full per-part step density is a non-finding.
</step_C3_audit_checklist>
...
- `critique_findings`: array of strings, at least one entry per audit item A1 through A6 (finding or "Non-finding: ..."), so it is never empty. Every finding is specific and evidence-backed: name the section, anchor, field, part, or quantity, and what the evidence shows. Brutal where warranted, never vague.
...
Every other field in lo_mapping.json is mapper-owned and read-only. If the draft lo_mapping.json parses but is missing mapper-owned fields or carries extra fields, do NOT try to repair its inventory: record the defect as a finding, fill your nine fields, and let the validator gate the package (output checklist items 2 and 3 are then judged against the draft as found). On any package with `mapping_confidence` "LOW" or `no_primary_available` true, missing_concepts must end up non-empty; if a weak mapping appears to be missing nothing, your A5 pass was too shallow, look again.
</step_C4_critique_fields>
...
4. `"critique_status": "completed"`; critique_score is an unquoted JSON number; critique_findings has at least one entry per audit item A1 through A6; primary_assessment, supporting_assessment, and multipart_assessment are non-empty and drawn from their vocabularies; search_reasonable is unquoted true or false.
