...
- `source_course_level`: the problem's course level as you judge it. Use the corpus's own course_level vocabulary (e.g., calculus_1) when a term from it honestly fits the problem; otherwise state the problem's true course level in the same lowercase snake_case style (e.g. linear_programming), never mislabeling an off-corpus problem with a corpus term.
