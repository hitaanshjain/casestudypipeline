...
- The phase2 v1 prompts have had ZERO test generations (drafted July 2, never run); the /phase1example fresh-session checklist in plan/phase2_prompt_design.md sec 4 is the first test, including the corrupted-verified_answer negative control for the calibration gate.
...
Next: (a) fresh-session test both prompts on /phase1example per the spec sec 4 checklist (case: compiles, calibration passes, fence holds, warning banner MUST fire on this LOW-confidence example, trap material, alignment cites Kuttler 1.3 / Austin 1.4 / Kuttler 1.2; flashcards: 5 cards, 2/2/1 mix, all cited); plus the negative control: corrupt one number in verified_answer.txt and the calibration gate must refuse; (b) re-map the sec 7 pipeline plan onto the two-phase split (schema freeze stays PAUSED, backlog 1b).
