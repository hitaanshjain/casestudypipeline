...
Progress: plan/phase2_prompt_design.md WRITTEN July 2 (full spec for both prompts: shared input contract and ground-truth hierarchy, calibration gate with exact error line, fence rule, fresh-numbers rule, derivation re-grounding, defensive weak-mapping warning triggers with the \documentclass-opening amendment, upgraded alignment block, flashcard sourcing/mix/traceability rules, test plan incl. the corrupted-verified_answer negative control). AWAITING Hitaansh's sign-off on the spec.
Then: (a) draft prompts/phase2_case_study_prompt_v1.md and prompts/phase2_flashcards_prompt_v1.md from the spec; (b) fresh-session test both on /phase1example; (c) re-map the sec 7 pipeline plan onto the two-phase split (schema freeze stays PAUSED, backlog 1b).
