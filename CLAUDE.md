...
## 14. ACTIVE TASK: write the phase 2 prompts (design settled July 2, spec sign-off pending)

All open questions settled with Hitaansh July 2:
1. FRESH numbers (see decisions log): same skills as the source problem, new clean-engineered instance; verified_answer.txt = calibration gate + method exemplar, never the case's key.
2. Flashcards are INDEPENDENT of the case study (used separately, no scenario references): default 5 cards, count is an input parameter, LaTeX output under the house preamble, grounded only in the 5 files with a per-card source anchor in comments; method concealment does not apply to flashcards.
3. TWO prompts, two calls: phase2 case study prompt and phase2 flashcards prompt, sharing the same input-digest and ground-truth rules.
4. Subtopic reversal confirmed and logged (sec 9); fence = problem + sections + missing_concepts.
5. Weak mapping (LOW mapping_confidence, primary_assessment needs_change, or multipart_missing_parts): proceed anyway; warning as a comment banner at the top of the .tex source plus an instructor-page note; student page stays clean (interpretation of Hitaansh's "top of the case study" answer, flagged to him).
6. Students see NONE of the 5 files (assume not course readings): the worksheet must be self-contained; sections serve as evidence of what students are assumed taught; missing_concepts print in the data block as reference data per P8.
7. CC BY 4.0 attribution: one-line credits auto-built from the files' metadata; case study carries them on the instructor page + source comments; the flashcards sheet (student-facing) carries a small credit line.
...
/prompts/phase2_case_study_prompt_v1.md     CURRENT, file-driven path (phase 2, from 5-file input)
